...
{
	/*
	 * The inlined DW_TAG_subprogram typically has the original source type
	 * for abstract origin of a concrete function with address range,
	 * inlined subroutine or call site.
	 */
	if (func->inlined)
		return false;
...
static struct tag *die__create_new_function(Dwarf_Die *die, struct cu *cu, struct conf_load *conf)
{
	...
	if (function != NULL) {
		/*
		 * For clang, we determine if function signature changes via
		 * DW_AT_calling_convention set to DW_CC_nocall.
		 */
