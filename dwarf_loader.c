				   ...
				   size_t exprlen, struct parameter *parm)
{
	...
	case DW_OP_consts:
		if (loc_num == 0) {
			...
			parameter__record_loc_const(parm, value,
						    ...
						    dwarf_op__const_signed(expr[0].atom),
						    false);
		}
		return;
	}
	...
	if (parm->type_byte_size <= cu->addr_size || !cu->agg_use_two_regs) {
		...
		case DW_OP_reg0 ... DW_OP_reg31:
			if (loc_num == 0)
				parameter__record_loc_reg(parm, expr[0].atom,
							  0, false, size);
			return;
		case DW_OP_breg0 ... DW_OP_breg31:
	...
	for (size_t i = 0; i < exprlen; i++) {
		if (expr[i].atom == DW_OP_piece) {
			...
			if (off < cu->addr_size || parm->loc_reg == PARAMETER_UNKNOWN_REG) {
				...
				parameter__record_loc_reg(parm, expr[i].atom,
							  0, false, size);
			}
		}
	}
				       ...
				       struct parameter *parm)
{
	...
	while ((offset = __dwarf_getlocations(attr, offset, &base, &start, &end, &expr, &exprlen)) > 0) {
		...
		case DW_OP_fbreg:
			if (loc_num == 0) {
				int64_t offset = dwarf_op__snumber(expr);

				parm->loc_stack = 1;
				if (offset >= INT32_MIN && offset <= INT32_MAX)
					parm->loc_offset = (int32_t)offset;
				parm->loc_size = size;
				parm->loc_deref = 1;
			}
			break;
		...
		case DW_OP_addr:
			if (loc_num == 0)
				parameter__record_loc_const(parm, expr->number,
							    cu->addr_size, false, true);
			break;
					     ...
					     int param_idx)
{
	...
	/*
	 * Inline expansion parameters are linked to exp->parms after the caller
	 * registers them in the CU tag table, so error paths can still delete an
	 * unlinked tag.
	 */
...
{
	/*
	 * Share location information between parameter and abstract origin; if
	 * neither have location, parameter analysis will mark it optimized out.
	 * Also share unexpected register use and true-signature state.
	 */
	if (parm->has_loc)
					 ...
					 struct ftype *ftype)
{
	...
	if (dparm->type == 0) {
		...
		/*
		 * Only concrete function parameters share analysis state back
		 * to their abstract origins. Inline expansion parameters are
		 * site-local: the same abstract parameter may be represented
		 * differently at each inline site.
		 */
		if (ftype != NULL)
...
static void inline_expansion__recode_dwarf_types(struct tag *tag, struct cu *cu)
{
	...
	/*
	 * DW_TAG_inlined_subroutine is special: the referenced subprogram is in
	 * the tag table, not the type table.
	 */
	if (dtag->type != 0)
		dtype = dwarf_cu__find_tag_by_ref(dcu, dtag, type);
	else
		dtype = dwarf_cu__find_tag_by_ref(dcu, dtag, abstract_origin);
	if (dtype == NULL) {
		if (dtag->type != 0)
			tag__print_type_not_found(tag);
		else
...
static int cu__resolve_func_ret_types_optimized(struct cu *cu, struct conf_load *conf)
{
	...
	for (i = 0; i < pt->nr_entries; ++i) {
		...
		/*
		 * Abstract inline subprograms describe source signatures, not a
		 * concrete call ABI site. Their per-site parameter locations are
		 * captured from DW_TAG_inlined_subroutine children instead.
		 */
		if (!fn->inlined) {
