...
static uint64_t btf__section_addr(const char *filename, const char *name)
{
	Elf *elf;
	Elf_Scn *scn = NULL;
	GElf_Ehdr ehdr;
	...
	if (fd < 0 || elf_version(EV_CURRENT) == EV_NONE)
		goto out_close;
				 ...
				 const struct btf_locsec_entry *entry)
{
	...
	site->section_addr = section_addr;
...
static void btf__mark_inline_functions(struct btf *btf, struct cu *cu)
{
	...
	for (type_index = 1; type_index < btf__type_cnt(btf); type_index++) {
		...
		/* LOCSECs are named "inline<section>", e.g. "inline.text". */
		...
		section_addr = btf__section_addr(cu->filename, name + strlen("inline"));
		...
		for (i = 0; i < nr_entries; i++) {
			...
			btf__add_inline_site(cu, name + strlen("inline"), section_addr,
					     &entries[i]);
		}
	}
}
