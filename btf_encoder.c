					     ...
					     struct inline_expansion *ie, uint64_t base)
{
	...
	offset = base && ie->ip.addr >= base ? ie->ip.addr - base : ie->ip.addr;
	if (offset > UINT32_MAX) { err = 0; goto out; }
	loc->offset = offset;
	...
	inline_expansion__for_each_parameter(ie, param) {
		if (param->idx < loc->nparams)
			btf_encoder__add_loc_param(param, base, &loc->params[param->idx]);
	}
...
static int btf_encoder__add_locsec(struct btf_encoder *encoder)
{
	...
	id = btf__add_locsec(encoder->btf, "inline");
	if (id < 0)
		return id;
	for (int i = 0; i < encoder->loc_states.cnt; i++) {
		struct loc *loc = states[i].loc;
		err = btf__add_locsec_loc(encoder->btf, loc->func_id, loc->loc_proto_id, loc->offset);
		if (err < 0)
			return err;
	}
	btf_encoder__delete_func_states(&encoder->loc_states);
...
int btf_encoder__encode_cu(struct btf_encoder *encoder, struct cu *cu, struct conf_load *conf_load)
{
	...
	if (conf_load->btf_gen_inlines) {
		for (size_t i = 1; i < encoder->seccnt; i++)
			if (encoder->secinfo[i].type == SHT_PROGBITS &&
			    strcmp(encoder->secinfo[i].name, ".text") == 0) {
